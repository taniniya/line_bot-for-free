...
async function sendLocalImage(buffer, filename) {
  const filePath = path.join(STATIC_DIR, filename)
  fs.writeFileSync(filePath, buffer)

  // Koyeb の場合、あなたのドメイン + /static/filename
  return `${process.env.PUBLIC_URL}/static/${filename}`
}
...
async function handleUserCommands(event, text) {
  ...
  if (trimmed === "/tenki") {
    try {
      ...
      const pops = area.areas[idx].pops[0]
      const temps = res.data[0].timeSeries[2].areas[idx].temps[0]
...
今日の運勢は…【${result}】`
  ...
  if (trimmed.startsWith("/image")) {
    try {
     ...
     // 画像をローカルの static フォルダに保存して URL を作る
     const filename = `quote_${Date.now()}.png`
     const filePath = path.join(STATIC_DIR, filename)
     fs.writeFileSync(filePath, buffer)

     // PUBLIC_URL は .env に自分のURLを入れておく（例: https://xxxx.koyeb.app）
          const baseUrl = process.env.PUBLIC_URL || ""
     const imageUrl = `${baseUrl}/static/${filename}`
     // LINE に画像を返す
     await lineClient.replyMessage(event.replyToken, {
       type: "image",
  ...
  if (trimmed === "/login") {
    try {
      ...
      if (Math.random() < 0.01) {
        ...
        coin = Math.floor(Math.random() * 251) + 50
      }
